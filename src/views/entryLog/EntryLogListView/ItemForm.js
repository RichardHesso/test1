...
const ItemForm = ({ className, closeItemForm, itemId, refreshResults, pmethodOptions, ...rest }) => {
  ...
  const editFields = [
    ...
    { id: 'user', label: 'Логин', type: 'text', variant: 'outlined', disabled: false, style: {marginTop: 10}, defaultValue: '' },
    { id: 'ip_address', label: 'IP', type: 'text', variant: 'outlined', disabled: false, style: {marginTop: 10}, defaultValue: '' },
    ...
    { id: 'created_at', label: 'Когда', type: 'text', variant: 'outlined', disabled: false, style: {marginTop: 10}, defaultValue: '' },
  ...
  const deleteItem = async (e, itemId) => {
    try {
      ...
      const response = await axios.delete(`/api/users/entry_logs/${itemId}/`);
  ...
  const getItem = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`/api/users/entry_logs/${itemId}/`);
    ...
    <Formik
      ...
      onSubmit={async (values, {
        ...
        try {
          ...
          if (itemId) {
            console.log(values)
            // response = await axios.patch(`/api/users/entry_logs/${itemId}/`, {
            ...
            response = await axios.post(`/api/users/entry_logs/`, {

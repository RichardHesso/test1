...
const PMethodListView = () => {

  const location = useLocation();
  const history = useHistory();

  const match = matchPath(location.pathname, {
    path: "/pmethods/:itemId",
    exact: true,
    strict: true
  });
  ...
  const [query, setQuery] = useState({ordering: '-id', page: 1, search: '', page_size: settings.page_size })
  ...
  const getItems = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`/api/finance/pmethods/?${queryPath}`);
...
export default PMethodListView;

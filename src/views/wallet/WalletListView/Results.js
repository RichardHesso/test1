...
const statusOptions = [
  ...
  {value: 'free', label: 'Cвободен'},
  {value: 'blocked', label: 'Занят'},
  {value: 'wait', label: 'В отлёжке'},
]

const Results = ({ className, items, query, setQuery, openItemForm, pmethodOptions, ...rest }) => {

  //console.log(6666666666, pmethodOptions)


  
  const getPMethodLabel = (pmethodId) => {
    return pmethodOptions.find(element => element.value === pmethodId).label
  };

  const headCells = [
    { id: 'id', label: '№', orderBy: 'id' },
    { id: 'payment_method', label: 'Платежный способ', orderBy: 'payment_method', choiceMap: getPMethodLabel },
    { id: 'wallet', label: 'Кошелек', orderBy: 'wallet' },
    { id: 'status', label: 'Статус', orderBy: 'status', choiceMap: getStatusLabel },
    { id: 'blocked_until_dt', label: 'Выдан до', orderBy: 'blocked_until_dt' },
    { id: 'when_waiting_dt', label: 'Снят с раздачи', orderBy: 'when_waiting_dt' },
  ];

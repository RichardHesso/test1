...
const ItemForm = ({ className, closeItemForm, itemId, refreshResults, pmethodOptions, ...rest }) => {
  ...
  const editFields = [
    ...
    { id: 'status', label: 'Cтатус', type: 'text', variant: 'outlined', options: statusOptions, disabled: false, style: {marginTop: 10}, defaultValue: 'working' },
    { id: 'time', label: 'Время', type: 'text', variant: 'outlined', disabled: false, style: {marginTop: 10}, defaultValue: '' },
    { id: 'days_of_week', label: 'Дни недели', type: 'text', variant: 'outlined', disabled: false, style: {marginTop: 10}, defaultValue: '' },
  ...
  const deleteItem = async (e, itemId) => {
    try {
      ...
      const response = await axios.delete(`/api/bot/periodic_compaigns/${itemId}/`);
  ...
  const getItem = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`/api/bot/periodic_compaigns/${itemId}/`);
    ...
    <Formik
      ...
      onSubmit={async (values, {
        ...
        try {
          ...
          if (itemId) {
            response = await axios.patch(`/api/bot/periodic_compaigns/${itemId}/`, {
            ...
            response = await axios.post(`/api/bot/periodic_compaigns/`, {
              mes_type: values.mes_type,
              text: values.text,
              file_id: values.file_id,
              time: values.time,
                  ...
                  <CardContent>
                    ...
                    >
                      ...
                      <Box mt={2}>
                        ...
                        <ConfirmDialog
                            title="Удалить периодическую рассылку?"
                            open={confirmOpen}
                            setOpen={setConfirmOpen}
                            onConfirm={e => deleteItem(e, itemId)}
                        >
                            Вы уверены что хотите удалить периодическую рассылку?
                        </ConfirmDialog>

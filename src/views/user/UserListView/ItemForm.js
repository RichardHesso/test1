...
const roleOptions = [
  ...
  {value: 'support2', label: 'Саппорт (кош, заявки)'},
]
...
const ItemForm = ({ className, closeItemForm, itemId, refreshResults, pmethodOptions, ...rest }) => {
  ...
  const editFields = [
    ...
    { id: 'role', label: 'Роль', type: 'text', variant: 'outlined', options: roleOptions, disabled: false, style: {marginTop: 10}, defaultValue: 'support' },
    { id: 'username', label: 'Логин', type: 'text', variant: 'outlined', disabled: false, style: {marginTop: 10}, defaultValue: '' },
    { id: 'passwd', label: 'Пароль', type: 'text', variant: 'outlined', disabled: false, style: {marginTop: 10}, defaultValue: '' },
  ...
  const addFields = [
    // { id: 'id', label: 'ID', type: 'number', variant: 'outlined', disabled: false, style: {marginTop: 10}, defaultValue: '' },
    { id: 'role', label: 'Роль', type: 'text', variant: 'outlined', options: roleOptions, disabled: false, style: {marginTop: 10}, defaultValue: 'support' },
    { id: 'username', label: 'Логин', type: 'text', variant: 'outlined', disabled: false, style: {marginTop: 10}, defaultValue: '' },
    { id: 'passwd', label: 'Пароль', type: 'text', variant: 'outlined', disabled: false, style: {marginTop: 10}, defaultValue: '' },
   //{ id: 'status', label: 'Cтатус', type: 'text', variant: 'outlined',  options: statusOptions, disabled: false, style: {marginTop: 20}, defaultValue: 'free' },
  ...
  const deleteItem = async (e, itemId) => {
    try {
      ...
      const response = await axios.delete(`/api/users/users/${itemId}/`);
  ...
  const getItem = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`/api/users/users/${itemId}/`);
    ...
    <Formik
      ...
      onSubmit={async (values, {
        ...
        try {
          ...
          if (itemId) {
            console.log(values)
            response = await axios.patch(`/api/users/users/${itemId}/`, {
              role: values.role,
            ...
            response = await axios.post(`/api/users/users/`, {
              role: values.role,
              username: values.username,
      ...
      {({
        ...
        >
          ...
          <Box mt={2}>
            ...
            >
                Вы уверены что хотите удалить пользователя {item.username}?
            </ConfirmDialog>

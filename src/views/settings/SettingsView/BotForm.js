...
const fields = [
    { id: 'is_active', label: 'Включен', type: 'checkbox', variant: 'outlined', disabled: false, style: {marginTop: 20}, defaultValue: true },
    { id: 'not_active_text', label: 'Текст когда бот выключен', type: 'multiline', variant: 'outlined', disabled: false, style: {marginTop: 20}, defaultValue: '' },
]
...
const BotForm = ({ className, ...rest }) => {
  ...
  const getItem = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`/api/settings/bot_config/1/`);
  ...
  return (
    ...
    <Formik
      ...
      onSubmit={async (values, {
        ...
        try {
            const response = await axios.patch(`/api/settings/bot_config/1/`, {
              is_active: values.is_active,
              not_active_text: values.not_active_text,
            });
